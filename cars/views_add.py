...
def add_car_for_client(request, client_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            Booking.objects.filter(car__isnull=True, plate_number=car.plate_number).update(car=car)
...
def edit_car_for_client(request, client_id, car_id):
    client = get_object_or_404(Client, id=client_id)
    car = get_object_or_404(Car, id=car_id, client=client)
    if request.method == 'POST':
    ...
    return render(request, 'edit_car_for_client.html', {'form': form, 'client': client, 'car': car})
