...
def booking_start_maintenance(request, booking_id):
    ...
    if booking.service_type:
        try:
            ...
            service = None
    # إذا كانت السيارة حالتها ليست قيد التنفيذ، يتم تحويلها إلى 'in_progress' عند بدء الصيانة
    if booking.car and booking.car.status != 'in_progress':
        booking.car.status = 'in_progress'
        booking.car.save()
...
def booking_create(request):
    ...
    if request.method == 'POST':
        ...
        plate_number = request.POST.get('plate_number') or (form.cleaned_data.get('plate_number') if form.is_valid() else None)
        if plate_number:
            from cars.models import Car
        ...
        form = BookingForm()
    return render(request, 'booking_form.html', {'form': form, 'client_info': client_info, 'car_info': car_info})
